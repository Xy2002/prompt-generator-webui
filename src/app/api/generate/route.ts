...
const METAPROMPT = `Today you will be writing instructions to an eager, helpful, but inexperienced and unworldly AI assistant who needs careful instruction and examples to understand how best to behave. I will explain a task to you. You will write instructions that will direct the assistant on how best to accomplish the task consistently, accurately, and correctly. Here are some examples of tasks and instructions.
...
export async function POST(request: NextRequest) {
  try {
    const { baseURL, apiKey, modelName, task, variables } = await request.json();
    ...
    if (variables && Array.isArray(variables) && variables.length > 0) {
      variableString = variables.map(v => `{$${v.toUpperCase()}}`).join('\n');
    }
    ...
    return Response.json(
      { error: '生成提示时发生错误: ' + (error instanceof Error ? error.message : '未知错误') },
      { status: 500 }
    );
  }
}